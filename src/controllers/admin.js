...
export const manageUpdateStatus = asyncHandler(async (req, res, next) => {
  ...
  if (!['approved', 'rejected'].includes(status)) {
    return next(new ErrorResponse('Invalid status. Only "approved" or "rejected" are allowed.', 400));
  }

  const update = await MissionUpdate.findById(req.params.id);

  if (!update) {
    return next(new ErrorResponse(`Update not found with id of ${req.params.id}`, 404));
  }
